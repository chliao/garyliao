...
void ChihShenxAna_oot(std::string fin, int processType=-1, float readoutWindow=3){ 
  ...
  for(int i=0;i<2;i++){
	...
	Long64_t nOOT3 = hdiff3[i][j]->GetEntries()-hr3[i][j]->GetBinContent(1);
        ...
        int lost1 = hdiff[i][j]->GetEntries()-hr[i][j]->GetEntries();
        errmc(lost1,
              hdiff[i][j]->GetEntries(),
              fraction, fraction_err,hlost[i],j);	
        int lost2 = hdiff2[i][j]->GetEntries()-hr2[i][j]->GetEntries();
        errmc(lost2,
              hdiff2[i][j]->GetEntries(),
              fraction, fraction_err,hlost2[i],j);        
        int lost3 = hdiff3[i][j]->GetEntries()-hr3[i][j]->GetEntries();
        errmc(lost3,
              hdiff3[i][j]->GetEntries(),
              fraction, fraction_err,hlost3[i],j);        
